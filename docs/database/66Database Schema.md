...
Records all script management activities for compliance and tracking (REQ-DTR-010). Designed for immutability and compliance with SOC 2 / ISO 27001 (REQ-DTR-020). Each entry is a self-contained document.
...
- Indexed for time-series queries. Consider TTL index for automatic data archival/deletion.
...
Dedicated log for script executions that violate sandbox constraints (REQ-DTR-001). This time-series data is crucial for security monitoring and compliance (REQ-DTR-020).
...
- Value from set: 'TIMEOUT', 'MEMORY_LIMIT', 'STATEMENT_COUNT'
...
- Correlation ID for a specific execution run.
